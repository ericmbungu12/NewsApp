...
import { CONTRACT_ADDRESS, BLOCKCHAIN_RPC, checkArticleOnChain, registerArticleOnChain, getNetworkType } from "./blockchain.ts";
...
const NO_BV      = (extra = {}) => ({ status:"not_registered", registered:false, verified:false, publisher:null, registered_at:null, contract_address:CONTRACT_ADDRESS??null, tx_hash:null, badge:"❌ Not on blockchain", ...extra });
...
serve(async (req: Request) => {
  ...
  try {
    ...
    for (const a of finalArticles.slice(0, 3)) {
      ...
      const reg = await registerArticleOnChain(a.description, a.url);
      if (reg.success && reg.txHash) {
        console.log(`✅ On-chain: ${reg.txHash}`);
        
        // ✅ Update Supabase with tx_hash
        await sbPatch(ENV.sbUrl, ENV.sbKey, `news?url=eq.${a.url}`, { tx_hash:reg.txHash, is_verified:true, blockchain_registered_at:new Date().toISOString() }, `tx_hash for ${a.url}`);
        
        // ✅ CRITICAL FIX: Sync in-memory article for immediate response
        // This ensures tx_hash appears in the current response, not just future queries
        (a as any).tx_hash = reg.txHash;
      } else console.warn(`⚠️ Registration failed: ${reg.error||"Unknown error"}`);
    }

    // ── Blockchain check per article ──────────────────────────────────────
    const articles = await Promise.all(finalArticles.map(async a => {
      const bv = a.description ? await checkArticleOnChain(a.description, a.content_hash) : null;
      const txHash = (a as any).tx_hash;
      const hasValidTxHash = typeof txHash === "string" && txHash.startsWith("0x") && txHash.length === 66;
      const isFullyRegistered = bv?.registered === true && hasValidTxHash;

      return {
        title:a.title, description:a.description??a.summary??"", url:a.url, image:a.image??null,
        source:a.source?.name??"Unknown", published_at:a.publishedAt??new Date().toISOString(),
        freshness_label:freshLabel(a.publishedAt), cluster_tag:a.tier??"general", relevance_score:a.relevance_score??0.85,
        blockchain_verification: isFullyRegistered
          ? { status:"registered", registered:true, verified:bv.verified, publisher:bv.publisher, registered_at:bv.timestamp, contract_address:CONTRACT_ADDRESS??null, tx_hash:tx_hash, badge:bv.verified?"✅ Verified on-chain":"⏳ Pending verification" }
          : NO_BV({ tx_hash:(a as any).tx_hash??null }),
      };
    }));
