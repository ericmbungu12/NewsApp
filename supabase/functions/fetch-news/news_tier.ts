...
// Combined rss_dynamic + news_tier. Imports from region.ts, fuzzy_search.ts, blockchain.ts
...
// ── Detect ISO from topic text using NAME_ISO from region.ts ──────────────────
...
// ── RSS XML parser — NO filtering, fuzzy_search.ts handles that ───────────────
...
// ════════════════════════════════════════════════════════════════════════════
// RSS SOURCES — all 8, all parallel
// ════════════════════════════════════════════════════════════════════════════

// S1: Google News RSS — universal, every country via region.ts
async function rssGoogle(topic:string,regionKey:string):Promise<any[]>{
  ...
  const raw=dedup((await Promise.allSettled([...urls].map(async u=>{try{const r=await fw(u,4000);if(!r.ok)return[];return parseXML(await r.text(),"Google News","google_news",4.0);}catch{return[];}})))
    .flatMap(r=>r.status==="fulfilled"?r.value:[]));
  const d=filterAndRank(core,raw,0.10);console.log(`✅ Google News: ${d.length}/${raw.length}`);return d;
}

// S2: Wikinews — 30+ languages via region.ts
async function rssWikinews(topic:string,regionKey:string):Promise<any[]>{
  ...
  const raw=dedup((await Promise.allSettled([...new Set([lang,"en"])].map(async l=>{
    try{const r=await fw(`https://${l}.wikinews.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(core)}&format=json&origin=*&srlimit=10`,4000);if(!r.ok)return[];
...
// S3: Regional aggregators — auto-selected from regionMap in region.ts
...
async function rssAggregators(topic:string,regionKey:string):Promise<any[]>{
  ...
  const raw=dedup((await Promise.allSettled([...groups].map(async g=>{const{url,name}=AGG[g];try{const r=await fw(url,4000);if(!r.ok)return[];return parseXML(await r.text(),name,"regional_agg",3.8);}catch{return[];}})))
    .flatMap(r=>r.status==="fulfilled"?r.value:[]));
  const d=filterAndRank(core,raw,0.10);console.log(`✅ Aggregators: ${d.length}/${raw.length} from ${groups.size} feeds`);return d;
}

// S4: Feedly discovery — finds RSS feeds for any topic
async function rssFeedly(topic:string):Promise<any[]>{
  const core=extractCoreEntity(topic);const feeds:string[]=[];
  try{const r=await fw(`https://feedly.com/v3/search/feeds?query=${encodeURIComponent(core+" news")}&count=8`,5000);if(r.ok)(await r.json()).results?.forEach((f:any)=>{const u=f.feedId?.replace(/^feed\//,"");if(u?.startsWith("http"))feeds.push(u);});}catch{}
  if(!feeds.length)return[];
  const raw=dedup((await Promise.allSettled(feeds.map(async url=>{try{const r=await fw(url,4000);if(!r.ok)return[];return parseXML(await r.text(),new URL(url).hostname.replace("www.",""),"discovered_feed",3.5);}catch{return[];}})))
    .flatMap(r=>r.status==="fulfilled"?r.value:[]));
  const d=filterAndRank(core,raw,0.10);console.log(`✅ Feedly: ${d.length} from ${feeds.length} feeds`);return d;
}

// S5: RSS Bridge — converts any site to RSS, covers remote places
async function rssBridge(topic:string):Promise<any[]>{
  ...
  ].map(async url=>{try{const r=await fw(url,6000);if(!r.ok)return[];return parseXML(await r.text(),"RSS Bridge","rss_bridge",3.5);}catch{return[];}})))
    .flatMap(r=>r.status==="fulfilled"?r.value:[]));
  const d=filterAndRank(core,raw,0.10);console.log(`✅ RSS Bridge: ${d.length}`);return d;
}

// S6: GDELT — hyper-local, every language, ~15min updates
async function rssGDELT(topic:string,regionKey:string):Promise<any[]>{
  ...
  const raw=dedup((await Promise.allSettled([core,core!==rn?`${core} ${rn}`:core].filter((v,i,a)=>a.indexOf(v)===i).map(async q=>{
    try{const p=new URLSearchParams({query:q,mode:"artlist",maxrecords:"25",format:"json",timespan:"48h",sort:"DateDesc"});const r=await fw(`https://api.gdeltproject.org/api/v2/doc/doc?${p}`,6000);if(!r.ok)return[];
...
// S7: Live & breaking feeds (news + sports)
...
async function rssLive(topic:string,isSport=false):Promise<any[]>{
  const core=extractCoreEntity(topic);
  const raw=dedup((await Promise.allSettled([...(isSport?SPORTS:[]),...LIVE].map(async({url,name})=>{try{const r=await fw(url,4000);if(!r.ok)return[];return parseXML(await r.text(),name,"live_feed",4.2);}catch{return[];}})))
    .flatMap(r=>r.status==="fulfilled"?r.value:[]));
  const d=filterAndRank(core,raw,0.10);console.log(`✅ Live: ${d.length}`);return d;
}

// S8: DuckDuckGo web search — last resort for very remote places
async function rssWebSearch(topic:string):Promise<any[]>{
  const core=extractCoreEntity(topic);const arts:any[]=[];
  try{
    const r=await fw(`https://api.allorigins.win/get?url=${encodeURIComponent(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(core+" news")}&ia=news`)}`,6000);
...
export async function fetchTier1NewsAPIs(queries:string[],regionKey:string,gnews?:string,newsapi?:string):Promise<any[]>{
  ...
  const results=(await Promise.allSettled(queries.map(async(q,i)=>{const sq=encodeURIComponent(q);const b:any[]=[];
    if(gnews){const r=await fw(`https://gnews.io/api/v4/search?q=${sq}&token=${gnews}&max=15&lang=en&country=${getCountryCode(regionKey)}`,4000).catch(()=>null);if(r?.ok)(await r.json()).articles?.forEach((a:any)=>b.push({...a,tier:"gnews",expansion_index:i}));}
    if(newsapi){const r=await fw(`https://newsapi.org/v2/everything?q=${sq}&language=en&pageSize=15&sortBy=publishedAt&apiKey=${newsapi}`,4000).catch(()=>null);if(r?.ok)(await r.json()).articles?.forEach((a:any)=>b.push({...a,tier:"newsapi",expansion_index:i,source:{name:a.source?.name??"NewsAPI"}}));}
    return b;
  }))).flatMap(r=>r.status==="fulfilled"?r.value:[]);
  const d=filterAndRank(core,dedup(results),0.10);console.log(`✅ T1 APIs: ${d.length}`);return d;
}

// T2: Wikipedia Current Events
...
// ════════════════════════════════════════════════════════════════════════════
// MASTER PIPELINE — all sources in parallel
// ════════════════════════════════════════════════════════════════════════════
...
// ════════════════════════════════════════════════════════════════════════════
// SYNTHESISE — real articles only, no placeholders
// ════════════════════════════════════════════════════════════════════════════
export function synthesizeWithGuaranteedCoverage(allArticles:any[],wiki:any,cleanTopic:string,_:string){
  ...
  const parts=[...(top3.length?[top3.join(" • ")]:[]),...(wiki?[`📚 **Background**: ${wiki.summary}`]:[]),...(real.length===0?[`ℹ️ No recent articles found for "${core}". Try searching in the local language.`]:[]),...(real.length>0&&real.length<3?[`💡 Limited coverage — ${real.length} source(s) found for "${core}".`]:[])];
...
// ════════════════════════════════════════════════════════════════════════════
// SUPABASE PERSISTENCE — attaches vector embeddings before insert
// ════════════════════════════════════════════════════════════════════════════
...
export async function insertArticlesToSupabase(url:string,key:string,articles:any[],topic:string,region:string,contractAddress:string|null){
  ...
  const rows=await Promise.all(withVectors.map(async a=>({url:a.url,title:a.title,description:a.description||a.summary||"",content:null,image:a.image??null,source:a.source?.name??"Unknown",published_at:a.publishedAt??new Date().toISOString(),published_ms:a.published_ms||new Date(a.publishedAt||0).getTime(),topic,region,category:a.category??null,freshness_label:fresh(a.publishedAt),cluster_tag:a.tier??"general",relevance_score:a.relevance_score??0.85,embedding:a.embedding??null,ai_summary:null,ai_reliability:null,content_hash:a.description?"0x"+await hashArticle(a.description):null,tx_hash:null,is_verified:false,publisher_address:null,blockchain_registered_at:null,contract_address:contractAddress,created_at:new Date().toISOString()})));
